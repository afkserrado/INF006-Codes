...
int main () {
    ...
    while (fgets(entrada, dimLinha, arqEntrada) != NULL) { 
        ...
        // Pega a lista LI (decimais)
        token = strtok_r(NULL, del1, &saveptr1); // Pega o restante após "LI"
        printf("Decimais = %s\n", token);

        // Verifica se a lista LI ou o subtoken estão vazios
        ...
        // Pega o primeiro subtoken da lista LE
        subtoken = strtok_r(token, del2, &saveptr2);

        // Pega os demais subtokens (decimais) da lista LI
        while (subtoken != NULL) {
